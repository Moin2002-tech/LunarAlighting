    ...
    void update(float dt, int action, const std::vector<float>& terrain_heights) {
        ...
        if (rocket_screen_x >= 0 && rocket_screen_x < SCREEN_WIDTH && !terrain_heights.empty()) {
            ...
            if (rocket_bottom <= terrain_y) {
                ...
                // If both legs are on ground and velocity is low, consider it landed
                if (left_leg_contact && right_leg_contact && std::abs(vy) < 0.5f && std::abs(vx) < 0.5f) {
                    vx = vy = angular_vel = 0.0f;
                    thrust_force = torque = 0.0f;
                    main_engine_on = left_engine_on = right_engine_on = false;
                    landing_timer += dt; // Start landing timer
                    
                    // Reset after 3 seconds of landing
                    if (landing_timer > 3.0f) {
                        should_reset = true;
                    }
                } else {
...
private:
    ...
    void generateMoonTerrain() {
        // Generate base terrain with some variation
        terrain_heights.resize(SCREEN_WIDTH + 1);
        
        std::random_device rd;
        std::mt19937 gen(42); // Fixed seed for consistent terrain
        std::uniform_real_distribution<float> height_variation(-0.2f, 0.2f);
        
        // Create smooth terrain using sine waves
        for (int x = 0; x <= SCREEN_WIDTH; x++) {
            float base_height = GROUND_HEIGHT;
            // Add some rolling hills
            base_height += 10 * std::sin(x * 0.01f);
            base_height += 5 * std::sin(x * 0.03f);
            // Add small variations
            base_height += height_variation(gen) * 3;
...
public:
    ...
    void drawLandingTarget() {
        ...
        SDL_FRect target = {static_cast<float>(target_screen_x - target_screen_width/2),
                           static_cast<float>(terrain_y - 5),
                           static_cast<float>(target_screen_width), 5.0f};
        ...
        SDL_RenderLine(renderer, target_screen_x - target_screen_width/2 - 10, terrain_y - 10,
                      target_screen_x - target_screen_width/2 - 10, terrain_y + 5);
        // Right marker
        SDL_RenderLine(renderer, target_screen_x + target_screen_width/2 + 10, terrain_y - 10,
                      target_screen_x + target_screen_width/2 + 10, terrain_y + 5);
    }
...
TEST_CASE("Lunar Alighting Simulation")
{
